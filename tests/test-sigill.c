...
int main(void)
{
    ...
    printf("test-sigill: test 2: NULL deref -> SIGSEGV... ");
    {
        struct sigaction sa;
        memset(&sa, 0, sizeof(sa));
        sa.sa_sigaction = sigsegv_handler_jmp, sa.sa_flags = SA_SIGINFO;
        sigemptyset(&sa.sa_mask);
        if (sigaction(SIGSEGV, &sa, NULL) < 0) {
            ...
            if (sigsetjmp(segv_jmp, 1) == 0) {
                /* Deliberate NULL deref to force SIGSEGV; the handler
                 * siglongjmps back here.
                 */
                volatile int *null_ptr = (volatile int *) 0;
                /* cppcheck-suppress nullPointer */
                (void) *null_ptr;
                /* If execution reaches this point, SIGSEGV was not delivered */
