 ...
 * still malformed past @n. Registers only failures, so a cell contributes
 * exactly one verdict: the pass comes from the oracle comparison, which
 * run_cell skips when this returns false.
 */
static bool check_structure(char **envp, int n, const char *cell)
{
    char detail[512];
                     ...
                     bool clear_env)
{
    ...
    int n = -1; /* must be left untouched on refusal */
    char **envp =
        guest_env_build(host_env, (char *const *) ovs, n_ovs, clear_env, &n);
    ...
    if (want.refused) {
        if (envp != NULL || n != -1) {
            snprintf(detail, sizeof(detail),
                     "%s: want refusal leaving *out_n untouched, got envp=%p "
                     "n=%d",
                     cell, (void *) envp, n);
    ...
    if (envp == NULL) {
        snprintf(detail, sizeof(detail), "%s: envp is NULL, want a vector",
                 cell);
        host_check(false, "build", detail);
        return;
    }

    if (!check_structure(envp, n, cell)) {
