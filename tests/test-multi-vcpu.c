...
/* Embedded shim binary (same blob that elfuse uses) */
#include "shim_blob.h"
...
#define SCTLR_RES1                                               \
    ((1ULL << 29) | (1ULL << 28) | (1ULL << 23) | (1ULL << 22) | \
     (1ULL << 20) | (1ULL << 11) | (1ULL << 8) | (1ULL << 7))
...
/* Build 3-level page tables mapping the current memory layout.
...
{
    uint64_t l0_off = pt_alloc(vm), l1_off = pt_alloc(vm);
    if (!l0_off || !l1_off)
        return 0;
...
/* Configure a vCPU with the same EL1 system-register state main.c installs
 * before the first hv_vcpu_run, so this standalone test exercises HVF
 * multi-vCPU support against the real shim entry contract.
 */

static int setup_vcpu(hv_vcpu_t vcpu,
                      uint64_t ttbr0,
                      uint64_t entry_ipa,
                      uint64_t sp_el0,
                      uint64_t sp_el1,
                    ...
                    int max_hvc5)
{
    ...
    for (int iter = 0; iter < 10000; iter++) {
        ...
        fprintf(stderr,
                "  unexpected exit reason=0x%x ec=0x%llx syndrome=0x%llx\n",
                vexit->reason,
...
typedef struct {
    vm_state_t *vm;
    uint64_t ttbr0, entry_ipa, sp_el0, sp_el1, shim_ipa;
...
static void *vcpu_thread(void *arg)
{
    ...
    if (setup_vcpu(vcpu, ctx->ttbr0, ctx->entry_ipa, ctx->sp_el0, ctx->sp_el1,
                   ctx->shim_ipa) < 0) {
...
#define _(insn) a64_##insn
    uint32_t code[] = {_(movz(0, 0)), _(movz(8, 93)), _(svc0())};
#undef _
    hvf_emit_code(vm.host_base, GUEST_CODE + CODE_A_OFF, code,
                  ARRAY_SIZE(code));
    hvf_emit_code(vm.host_base, GUEST_CODE + CODE_B_OFF, code,
                  ARRAY_SIZE(code));

    /* Launch two threads, each running a vCPU */
    thread_ctx_t ctx_a = {.vm = &vm,
                          .ttbr0 = ttbr0,
...
#define _(insn) a64_##insn
    {
        uint32_t code[] = {
            _(movz(0, 0xAA)),
            _(movz(1, (uint16_t) (GUEST_DATA & 0xFFFF))),
            _(movk_lsl16(1, (uint16_t) ((GUEST_DATA >> 16) & 0xFFFF))),
            _(str_imm64(0, 1, 0)), /* str x0, [x1, #0] */
            _(movz(0, 0)),
            _(movz(8, 93)),
            _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_A_OFF, code,
...
#define _(insn) a64_##insn
    {
        uint32_t code[] = {_(movz(8, 0xAAAA)), _(svc0()), _(movz(0, 0)),
                           _(movz(8, 93)), _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_A_OFF, code,
                      ARRAY_SIZE(code));
    }
    ...
    {
        uint32_t code[] = {_(movz(8, 0xBBBB)), _(svc0()), _(movz(0, 0)),
                           _(movz(8, 93)), _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_B_OFF, code,
                      ARRAY_SIZE(code));
    }
...
#define _(insn) a64_##insn
    {
        uint32_t code_a[] = {_(movz(19, 0x1234)), _(movz(20, 0x5678)),
                             _(movz(8, 0xAAAA)),  _(svc0()),
                             _(mov_reg(0, 19)),   _(mov_reg(1, 20)),
                             _(movz(8, 93)),      _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_A_OFF, code_a,
                      ARRAY_SIZE(code_a));
    }
    {
        uint32_t code_b[] = {_(movz(19, 0x1234)), _(movz(20, 0x5678)),
                             _(movz(8, 0xBBBB)),  _(svc0()),
                             _(mov_reg(0, 19)),   _(mov_reg(1, 20)),
                             _(movz(8, 93)),      _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_B_OFF, code_b,
                      ARRAY_SIZE(code_b));
    }
#undef _
...
static void *vcpu_thread_tlbi_a(void *arg)
{
    ...
    vcpu_exit_t ex;
    if (run_vcpu(vcpu, vexit, &ex, 0) < 0 || ex.reason != HVF_EXIT_HVC5) {
...
#define _(insn) a64_##insn
    ...
    {
        uint32_t code[] = {
            _(movz(1, (uint16_t) (TLBI_REGION & 0xFFFF))),
            _(movk_lsl16(1, (uint16_t) ((TLBI_REGION >> 16) & 0xFFFF))),
            _(ldr_imm64(0, 1, 0)), /* ldr x0, [x1] */
            _(movz(8, 93)), _(svc0())};
        hvf_emit_code(vm.host_base, GUEST_CODE + CODE_B_OFF, code,
                      ARRAY_SIZE(code));
    }
...
int main(void)
{
    ...
    for (int i = 0; i < ntests; i++) {
        ...
        if (rc == 0) {
            ...
            /* If test 1 fails, stop because multi-vCPU is not viable */
            if (i == 0) {
                printf(
                    "\nTest 1 failed -- multi-vCPU not viable in HVF. "
                    "Stopping.\n");
                break;
            }
        }
