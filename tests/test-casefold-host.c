...
/* The UTF-16 code units @s occupies, which is what the volume counts against
 * its per-name limit. A second implementation of the codec's arithmetic, so a
 * change breaking the unit budget shows up as a disagreement here rather than
 * in both sides at once. 0 means @s is not well-formed UTF-8.
 */
