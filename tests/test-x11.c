...
static int parse_display(const char *display, display_info_t *info)
{
    ...
    if (!colon || colon == display + strlen(display) - 1)
        return -1;
    ...
    if (host_len == 0) {
        /* ":N" means standard Unix socket at /tmp/.X11-unix/XN */
        ...
        /* If path-prefix does not exist, use full DISPLAY string */
        if (access(info->socket_path, F_OK) != 0)
            snprintf(info->socket_path, sizeof(info->socket_path), "%s",
                     display);
    } else {
...
static int find_xauth(int display_num, xauth_t *auth)
{
    ...
    while (1) {
        ...
        if (xauth_read_be16(fd, &family) < 0)
            break;
        if (xauth_read_be16(fd, &addr_len) < 0)
            break;
        if (xauth_skip(fd, addr_len) < 0)
            break;
        if (xauth_read_be16(fd, &number_len) < 0)
            break;

        /* Read display number (ASCII string) */
        char numbuf[32] = {0};
        if (number_len > 0 && number_len < sizeof(numbuf)) {
            if (read(fd, numbuf, number_len) != number_len)
                break;
        } else if (number_len > 0) {
            if (xauth_skip(fd, number_len) < 0)
                break;
        }

        /* Read auth protocol name */
        if (xauth_read_be16(fd, &name_len) < 0)
            break;
        char namebuf[64] = {0};
        if (name_len > 0 && name_len < sizeof(namebuf)) {
            if (read(fd, namebuf, name_len) != name_len)
                break;
        } else if (name_len > 0) {
            if (xauth_skip(fd, name_len) < 0)
                break;
        }

        if (xauth_read_be16(fd, &data_len) < 0)
            break;

        /* Match on display number */
        bool match =
            ((int) number_len == display_str_len &&
             !memcmp(numbuf, display_str, display_str_len) &&
             name_len < sizeof(auth->name) && data_len <= sizeof(auth->data));
        if (match) {
            memcpy(auth->name, namebuf, name_len);
            auth->name_len = name_len;
            auth->data_len = data_len;
            if (data_len > 0 && read(fd, auth->data, data_len) != data_len)
                break;
            close(fd);
            return 0;
        }
        if (xauth_skip(fd, data_len) < 0)
            break;
    }
    close(fd);
    return -1;
...
typedef struct {
    ...
    uint32_t root_window, root_visual;
    uint8_t root_depth;
                       ...
                       x11_conn_t *conn)
{
    ...
    if (info->type == DISPLAY_UNIX) {
        ...
        if (!strcmp(info->hostname, "localhost") ||
            !strcmp(info->hostname, "127.0.0.1"))
            addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
        else if (inet_pton(AF_INET, info->hostname, &addr.sin_addr) <= 0)
            addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
        if (connect(fd, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
            close(fd);
            return -1;
    ...
    if (auth_data_len > 0)
        memcpy(setup + 12 + pad4(auth_name_len), auth->data, auth_data_len);
    ...
    uint8_t num_screens = data[20], num_formats = data[21];
    if (num_screens == 0) {
        close(fd);
        return -1;
    }
...
static int x11_create_gc(x11_conn_t *c,
                         uint32_t gcid,
                         uint32_t drawable,
                         uint32_t fg)
{
...
static int x11_create_window(x11_conn_t *c,
                             uint32_t wid,
                             uint32_t parent,
                             int16_t x,
                             int16_t y,
...
static int x11_fill_rect(x11_conn_t *c,
                         uint32_t drawable,
                         uint32_t gc,
                         int16_t x,
                         int16_t y,
...
int main(void)
{
    ...
    {
        display_info_t d;
        if (parse_display(":0", &d) == 0 && d.type == DISPLAY_UNIX &&
            d.display_number == 0 &&
            !strcmp(d.socket_path, "/tmp/.X11-unix/X0"))
            PASS();
        else
            FAIL("wrong parse");
    }
    ...
    {
        display_info_t d;
        if (parse_display(":1.2", &d) == 0 && d.display_number == 1 &&
            d.screen_number == 2)
            PASS();
        else
            FAIL("wrong parse");
    ...
    {
        display_info_t d;
        if (parse_display("localhost:0", &d) == 0 && d.type == DISPLAY_TCP &&
            !strcmp(d.hostname, "localhost") && d.display_number == 0)
            PASS();
        else
            FAIL("wrong parse");
    }
    ...
    {
        display_info_t d;
        EXPECT_TRUE(parse_display(NULL, &d) < 0 && parse_display("", &d) < 0,
                    "should reject");
    }
    ...
    TEST("X11 socket connect");
    x11_conn_t conn;
    int connected = (x11_connect(&dinfo, have_auth ? &auth : NULL, &conn) == 0);
    ...
    printf("OK (root=0x%x depth=%d)\n", conn.root_window, conn.root_depth);
    passes++;

    /* Create window, map it, wait for Expose, draw a rectangle */
    TEST("X11 window + draw");
    {
        uint32_t wid = x11_alloc_id(&conn), gcid = x11_alloc_id(&conn);
        ...
        if (x11_create_window(&conn, wid, conn.root_window, 100, 100, 200, 200,
            ...
            for (int i = 0; i < 50 && !ok; i++) {
                ...
                if (type == -2)
                    continue; /* reply; skip */
                if (type <= 0)
                    break; /* transport error or X error */
            ...
            EXPECT_TRUE(ok, "no Expose event within 5s");
        }
    }
