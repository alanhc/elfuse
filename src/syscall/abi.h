 ...
 *
 * Just the SYS_* dispatch numbers and the dispatch entry points. Everything
 * else that used to live here (errno values, flag constants, wire-format
 * structs, the FD table) moved to syscall/linux-wire.h. Almost every syscall
 * implementation file needs that half and never touches SYS_* at all, so
 * splitting them keeps a change to one from forcing a rebuild of files that
 * only wanted the other.
 *
 * The SYS_* numbers have exactly two readers: syscall.c, and
 * debug/syscall-hist.c through the generated dispatch.h. Everything else that
 * includes this header (core/bootstrap.c, runtime/fork-state.c, syscall/proc.c)
 * wants only the two entry-point declarations at the bottom, and includes
 * linux-wire.h separately for the wire half.
 */
